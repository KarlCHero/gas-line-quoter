  ...
  {:else}
    <div class="head">
      <div>
        ...
        <div class="meta">Natural Gas · Copper · AS/NZS 5601 · Ex GST</div>
      </div>
    ...
    <div class="card">
      ...
      <table>
        <thead><tr><th class="l">Section</th><th>Length</th><th>Flow</th><th>Size</th><th>Material Cost</th></tr></thead>
        <tbody>
          {#each qr.sized as s, i (s.id)}
            <tr>
              ...
              <td>
                ...
                {#if s.overCapacity}<span class="flag over" title="Exceeds DN50 capacity at this run length — needs design review or higher supply pressure">⚠ over</span>
                {:else if s.oversized}<span class="flag up" title="Stepped up a size — borderline at the smaller size">↑ oversized</span>{/if}
              </td>
              <td>{s.length ? fmt(s.length * (cfg.copperRates[s.size] || 25)) : '—'}</td>
            </tr>
          {/each}
        </tbody>
      </table>
      <div class="tablenote">Sized per AS/NZS 5601.1:2013 Appendix F — copper (AS 1432 Type B), Table {qr.band.id} ({qr.band.dropKPa} kPa drop, supply {qr.band.supplyRange}), longest-run method.</div>
    ...
    <div class="card">
      ...
      <div class="rp indent"><span>Copper{#if cfg.pipeWastePct} <span class="muted">· incl. {cfg.pipeWastePct}% waste</span>{/if}</span><span>{fmt(qr.copperMat)}</span></div>
